...
-- Черновая замена графики корпуса и башни танка на собственную (сгенерированную
-- программно, без художника). 16 направлений вместо ванильных 64 -- поворот
-- будет заметно более "ступенчатым", это ожидаемо для черновика.
-- Проверить в игре не удалось (нет доступа к Factorio) -- если размер машины
-- визуально не совпадает с её collision_box, подправьте "scale" ниже.
sau_vehicle.animation = {
  filename = "__sau-artillery__/graphics/entity/sau-artillery-hull.png",
  priority = "low",
  width = 128,
  height = 128,
  frame_count = 1,
...
sau_vehicle.turret_animation = {
  filename = "__sau-artillery__/graphics/entity/sau-artillery-turret.png",
  priority = "low",
  width = 128,
  height = 128,
  frame_count = 1,
